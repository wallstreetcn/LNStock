...
@property (nonatomic, strong) LNStockView *footView;
...
- (LNStockView *)stockView {
    if (!_stockView) {
        ...
        _stockView.isGreenUp = [[PListUtils valueForKey:@"IsGreenUp"] intValue];  //是否为绿涨红跌
        _stockView.quotesViewDataBlock = ^(LNStockModel *model) {
            //实时刷新行情数据
        };
    }
    return _stockView;
...
- (LNStockView *)footView {
    if (!_footView) {
        //添加K线图
        _footView = [LNStockView createViewWithCode:@"000300.SS" isAstock:YES isNight:self.isNightMode];
        _footView.quotesViewDataBlock = ^(LNStockModel *model) {
            //实时刷新行情数据
        };
    }
    return _footView;
...
- (UITableView *)priceTableView {
    if (!_priceTableView) {
        _priceTableView = [[UITableView alloc] initWithFrame:CGRectMake(0,44, kFBaseWidth, kFBaseHeight)];
        _priceTableView.tableHeaderView = self.stockView;
//        _priceTableView.tableFooterView = self.footView;
...
- (void)getStockData {
    ...
    [LNStockNetwork getStockRealDataWithCode:self.code isAstock:self.isAstock block:^(BOOL isSuccess, LNStockModel *data) {
        if (isSuccess) {
            //刷新数据
        }else {
            NSLog(@"行情头部失败");
